...
const views = [PrimeList, StackView, SpectrumView, WaveView, FlowView, CrtView, GapView];
let stack = null;
...
function readout() {
  ...
  host.innerHTML = '';
  const chip = (k, v, title) => host.appendChild(
  ...
  chip('survivors/window =', `${stack.survivors}/${stack.N} = ${fmt(stack.survivors / stack.N, 6)}`);
  chip('H_joint =', `${fmt(stack.Hjoint, 4)} bits`, 'Σ H(p_i): additive by CRT independence');
  chip('h_k =', `${fromLog10(last ? last.log10EntropyDensity : NaN)} b/int`, 'joint entropy per integer of period');
  chip('H_out =', `${fmt(last ? last.outputEntropy : 0, 4)} bits`, 'entropy of the single output bit S_k(n) → 0');
  chip('nonzero harmonics =', String(stack.stages.reduce((a, s) => a + s.combLines, 0)));
  chip('exact from H ≥', String(harmonicsForExactness(stack.basis)), 'harmonics needed for the wave view to be exact');
}

function bindControls() {
  const k = qs('#k'), kOut = qs('#kOut');
  k.addEventListener('input', () => {
    state.k = +k.value; kOut.textContent = k.value; recompute();
  });

  qs('#window').addEventListener('change', e => {
    state.windowLength = +e.target.value; recompute();
  });
  ...
  const H = qs('#harmonics'), hOut = qs('#hOut');
  H.addEventListener('input', () => {
    state.harmonics = +H.value; hOut.textContent = H.value;
    WaveView.render(stack, state); readout();
  });

  qs('#showRaw').addEventListener('change', e => {
    state.showRaw = e.target.checked; StackView.render(stack, state);
  });

  qs('#logSpectrum').addEventListener('change', e => {
    state.logSpectrum = e.target.checked; SpectrumView.render(stack, state);
  });
