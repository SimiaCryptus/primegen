...
let canvas = null, selX = null, selY = null, info = null, app = null;
let ix = 0, iy = 1, lastSelected = -1, last = null;

export function mount(ctx) {
  app = ctx;
  canvas = qs('#crtCanvas');
  selX = qs('#crtX'); selY = qs('#crtY'); info = qs('#crtInfo');
  selX.addEventListener('change', () => { ix = +selX.value; redraw(); });
  selY.addEventListener('change', () => { iy = +selY.value; redraw(); });
}

function redraw() { if (last) draw(last.stack, last.state); }
...
export function render(stack, state) {
  ...
  ix = Math.min(ix, n - 1); iy = Math.min(iy, n - 1);
...
function draw(stack, state) {
  const p = stack.basis[ix], q = stack.basis[iy];
  const { ctx, w, h } = fitCanvas(canvas, 230);
  ctx.fillStyle = '#0a0d13';
  ctx.fillRect(0, 0, w, h);
  const pad = 16;
  const plotW = w - 2 * pad, plotH = h - 2 * pad;
  const cw = plotW / p, ch = plotH / q;

  for (let a = 0; a < p; a++) {
    for (let b = 0; b < q; b++) {
      const deadP = a === 0, deadQ = b === 0;
      let color;
      ...
      ctx.fillStyle = color;
      ctx.fillRect(pad + a * cw, pad + b * ch, Math.max(0.7, cw - (cw > 4 ? 1 : 0)),
