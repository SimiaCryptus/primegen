...
A third form (**Algorithm C**, §4C) refines Algorithm B along the _exponent spine_: it indexes streams by `(p, e)` and
owns `m` through its min-factor normal form `m = p^e·r` with `p = spf(m)`, `e = v_p(m)` and `r` coprime to every prime
`≤ p`. Its touch count equals B's to within `Θ(N^{2/3}/log N)`, its claimant sets are strictly smaller, and it emits
`(spf(m), v_p(m), m/p^e)` for every composite it rejects — a streaming min-factor oracle rather than a bare primality
decision. It is specified and analysed in `min_factor.md`.
...
reading; `min_factor.md` specifies Algorithm C, the exponent-spine refinement and its min-factor output; `theory.md` is
the statement-by-statement inventory, with status, dependency graph and prior art. All seven describe one object and
use one vocabulary.
...
> set is `Θ_p = p·A_p` read mod `P_{k+1}`—`Primegen.theta*eq_image`.*
...
### 2.6 The exponent refinement: min-factor normal form

Claim O1 splits `m` at its smallest prime factor **once**. Splitting there _repeatedly_ — i.e. taking the full `p`-adic
valuation — refines the same partition one step further, at no mathematical cost, and it is that refinement which
Algorithm C streams. Write

```
B_p = { r ≥ 1 : q prime, q ∣ r ⟹ q > p }        (the monoid generated by the primes > p;  B_p = {1} ⊎ R_{>p})
```

> **Claim M1 (min-factor normal form).** Every `n > 1` is **uniquely** `n = p^e·r` with `p` prime, `e ≥ 1`, `r ∈ B_p`,
> and then `p = spf(n)`, `e = v_p(n)`, `r = n/p^e`. Hence
> `A_p = (B_p \ {1}) ⊎ ⨄_{e≥1} p^e·B_p` and `Θ_p = (⨄_{e≥1} p^e·B_p) \ {p}`.
>
> _Status: unique factorisation, re-bracketed; not yet formalised — Open Problem 11._
> **Claim M2 (cone recursion).** `B_p = {1} ⊎ ⨄_{q>p prime} q·B_q`. So the multiplier cloud of `p` is _generated by the
> clouds of the larger primes_: coprimality is a consequence of where one enters the factor tree, not a precondition
> tested on the way in. _Status: routine; not formalised._
> **Claim M3 (strict phase separation).** If `r ∈ B_p` and `1 < r < (p+1)²` then `r` is prime. Hence a composite
> multiplier in layer `(p,e)` forces the emitted value `≥ p^{e+2}`: layer `e` has a rough phase below `N` only for
> `p ≤ N^{1/(e+2)}`. _Status: the argument of `prime_of_rough_of_lt_sq`, one step stricter._
> Three warnings, since this reframing is easy to over-read. It does **not** remove coprime sets: deciding `r ∈ B_p` is
> deciding `gcd(r, P_{≤p}) = 1`, so Conjecture X1 applies verbatim with `P_{≤p}` for `P_{<p}`. And it does **not** change
> the partition of §2.1 — summing the layers of `p` re-assembles `p·A_p = Θ_p`. What it changes is the _index_, and with
> it the output: see §4C.
> Finally, M3 is a statement about the **exact** cloud `B_p` and does **not** survive the relaxation that Algorithm C
> actually runs on: `B̃_p` (§4C.1) is a `W`-coprime wheel containing composites — `49 = 7² ∈ B̃_11` at `W = 30`, giving
> `11·49 = 539 = 7²·11 < 11³` — so a layer's multipliers below `p²` are not primes, advancement is a wheel step rather
> than a prime-index increment, and `r ≥ p` does not force `spf(r) > p`. See `min_factor.md` Remark M-R.
...
> emit branch `p_k ≤ n/2` so `p_{k+1} < n`by Bertrand; in the spawn branch`p*k ≤ √(n/2)`. TODO.*
...
## 4C. Algorithm C — the exponent-spine generator

Algorithm C is Algorithm B re-indexed by Claim M1: streams are **layers** `(p, e)` rather than primes `p`. It is
specified in full, with proofs, costs and the reference implementation, in `min_factor.md`; this section is the
summary, and §6 carries its row.

### 4C.1 Design

Same wheel `W`, same candidate scan, same deferred activation. For `p > p_w` replace the exact cloud `B_p` by

```
B̃_p = {1} ∪ { r ≥ p : gcd(r, W) = 1, p ∤ r }  ⊇  B_p ,
```

and let layer `(p, e)` be the stream `p^e·B̃_p`, minus the single non-composite element `p`. Both retained constraints
matter: `r ≥ p` prevents a layer from claiming numbers whose smallest factor is `r`, and `p ∤ r` keeps the layers of one
prime disjoint from each other (without it a prime power `p^k` is claimed `k` times — the naive reading of the idea, and
the one that costs). Note what `r ≥ p` does _not_ buy: `B̃_p` is a wheel and not a prime list, so it contains every
`W`-coprime composite `≥ p` whose factors all exceed `p_w`, and `spf(r) > p` can fail — layer `(11,1)` at `W = 30`
takes `r = 49` and emits `539 = 7²·11`, owned by `7`. Claim M3 therefore does not transfer, and C over-claims exactly
as B does; that is what Claim M10 counts (`min_factor.md` Remark M-R). `p ∤ r` is enforced by a **skip cursor**: one extra word holding a multiple of `p`, advanced by
`+p`, which adds strictly less than one increment per advance for every `p > p_w ≥ 13`. Heads are `p^e` for `e ≥ 2` and
`p·r₁(p)` for `e = 1`, all `≥ p²`, so activation still fires exactly at `n = p²`; a layer's cloud is opened by the pop
of its own spine element `p^e`, so no global `N` decides how many layers a prime gets.

### 4C.2 Claims

> **M6 (coverage).** Every `W`-coprime composite `m` is a value of layer `(p, e)` with `p = spf(m)`, `e = v_p(m)`.
> **M7 (soundness).** Every key is a `W`-coprime composite. **M8 (no early claims).** Every key is `≥ n` and `≥ p²`.
> **M9 (decision rule).** `n` is prime iff no key equals `n`; `C()` emits exactly the primes, forever.
>
> _Status: proved in `min_factor.md` §6; not machine-checked. They are the exact analogues of `AlgB.coverage`,
> `AlgB.not_prime_of_claims`, `AlgB.Inv.ahead` and `AlgB.emit_iff`._
> **Claim M10 (duplicate accounting).** For a `W`-coprime composite `m`,
> `claimants_C(m) = { (p, v_p(m)) : p ∣ m, and ( m = p^{v_p(m)} or m/p^{v_p(m)} ≥ p ) }`, one layer per claiming prime,
> and `claimants_C(m) ⊆ claimants_B(m) = { p : p ∣ m, p² ≤ m }`. Both contain `spf(m)`. Smallest strict witness at
> `W = 30`: `847 = 7·11²` is claimed twice by B (`7·121`, `11·77`) and once by C (layer `(7,1)`, `r = 121`).
> **Claim M11 (min-factor oracle).** The claimant with the smallest `p` carries `(spf(n), v_p(n), n/p^{v_p})`. Every
> rejected candidate is therefore reported in min-factor normal form, at no cost beyond the drain that already happens.

### 4C.3 Cost, honestly

Summing Claim M10 over layers gives `κ_W·N/p` per prime — Algorithm B's figure, since the layers merely partition the
multiples B handles in one stream. The whole difference is the discarded tail `r < p`, paid once per layer instead of
once per prime:

```
ops_C(N) = κ_W·N·( ln ln √N − ln ln p_w + o(1) ) − Θ(N^{2/3}/log N) .
```

At `N = 10¹²`, `w = 6`, that gap is `≈10⁻⁴·N` against `ops_B ≈ 0.46·N`. **Algorithm C is not a faster prime generator.**
Memory is `2π(√n) + O(n^{1/3})` records of four words in the uniform form of `min_factor.md` §5, and
`(1+o(1))·π(√n)` in the thinned form (handle `p²` inline at activation, gate the rest of the spine on a second
activation cursor at `p³ ≤ n`). Random-access restart survives layer by layer, so segment parallelism is exactly B's.
The case for Algorithm C is therefore not touch count but **output**: a streaming `(spf, v_p, cofactor)` oracle — hence
streaming `ω`, `Ω`, `μ`, radical, prime powers (the spine records _are_ the prime powers, in order) and smooth/rough
enumeration by layer selection rather than by test. Algorithm B recovers `spf` (its minimum claimant, since
`spf(m)² ≤ m`) but not the valuation; Algorithm A gives the _largest_ prime factor, the harder end to iterate on.
Read on the lattice (§2.5), Claim M1 says the copy deleted at each stage of F1 is itself an infinite geometric tower
`⨄_{e≥1} p^e·B_p` at ratios `p^{-e}`, and Claim M2 makes the system graph-directed with the primes as nodes — see
`min_factor.md` §9, including the warning that no dimension may be read off that grammar.
...
| **Algorithm C**                                                                                                       | `ops_B(N) − Θ(N^{2/3}/log N)`, same leading term                | `O(π(√N) + W)`, 4-word records (thinned spine)           | no (heap) / segment (bucket) | **yes**          |
...
| the tabulated depth\_: an array over the range (wheel sieve), a `ln ln` duplicate factor (Algorithm B), or `N^{0.75}`   |
...
11. **Formalise the exponent refinement.** Claims M1 and M10 (§2.6, §4C): `MinFac.normal_form`, `AlgC.coverage`,
    `AlgC.claims_iff`, and the inclusion `claimants_C ⊆ claimants_B` — the statement that makes "C is B with a finer
    index" precise. M1 is `Nat.factorization` / `ord_proj`–`ord_compl` in Mathlib terms and should be short.
12. **Exponent-spine Algorithm A.** Apply the `(p, e)` index to Algorithm A's bases, `b = p^e·b'`. Does run-length
    encoding the spine suppress the smooth bases that dominate `S(N)`, as the `w`-wheel of §3.5 does?
13. **How much factorisation is the min-factor stream worth?** By Claim M4 the cofactor of `n` is `< n`, so a bounded
    window of previously-reported normal forms fully factors `n`. How large a window, as a function of the range, and is
    it ever cheaper than re-running the stream? (The unbounded version is a range-proportional array, which is X1
    again — `min_factor.md` §2.5.)
...
One further index is free. Splitting at the smallest prime factor _with its multiplicity_ (Claim M1) refines each stream
into layers `p^e·B_p`, which costs nothing structurally — on the lattice it is the observation that the deleted Moran
copy is itself geometric — and turns the generator into a streaming `(spf, v_p, cofactor)` oracle whose unclaimed
residue is the primes (§4C, `min_factor.md`). Touch count is unchanged to four significant figures; what changes is what
the generator knows about the numbers it rejects.
...
- **All of Algorithm C** (§4C, `min_factor.md`): the normal form M1, the cone recursion M2, coverage M6, soundness M7,
  the invariant M8, the decision rule M9, the claimant identity M10 and the oracle M11. Every one of them is the
  analogue of something already checked for Algorithm B, so this is scaffolding work rather than new mathematics; it is
  Open Problem 11, and M10's inclusion `claimants_C ⊆ claimants_B` is the piece worth doing first.
...
1. **A wheel does not decide primality — it enumerates candidates.** By Lemma 2.3 the holes of the wheel of modulus
   \(P*{<p}\) are certified prime exactly on \([p, p^2)\); the first failure is \(121 = 11^2\), a hole of the mod-\(210\)
   wheel \_inside its own first period* (Cor. 2.4). Hence a wheel sieve is always **wheel + composite removal**, and all
   of its correctness lives in the second half. Deciding all of \([2,N]\) from wheel membership alone would require
   modulus \(P*{<\sqrt N} = e^{ (1+o (1))\sqrt N}\) — superpolynomially more space than the answer occupies. The
   construction is non-circular only because it respects the window: extending \(S_k\) requires \(p*{k+1}\), the least
...
2. **The \(\log\log\) is Mertens, and it is a ceiling on the whole family.** Touches scale with the candidate density
   \(\kappa*k = \varphi (P_k)/P_k \sim e^{-\gamma}/\log p_k\), so the bound is attained only by letting the modulus grow
   ...
   wheel beats \(\log\log\), and no \_storable* wheel beats a small constant — which is why real sieves live at
...
4. **A fixed wheel is this algorithm, truncated.** The wheel-extension step is exactly the recursion
   \(S*{k+1} = (p\text{ tilings of } S_k)\setminus p\cdot S_k\) whose deleted set is \(\Theta_p = p\cdot A_p\)
   ...
   precisely the \(\omega*{>p_w}\) duplicate factor of §4.5 for the reuse. **Algorithm A** runs it to full depth with no
