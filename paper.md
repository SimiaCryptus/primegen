...
> Two warnings, since this reframing is easy to over-read. It does **not** remove coprime sets: deciding `r ∈ B_p` is
...
the one that costs). `p ∤ r` is enforced by a **skip cursor**: one extra word holding a multiple of `p`, advanced by
