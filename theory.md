...
`generator.md`, `observation.md`, and `idea.md` either **introduces**, **assumes**, or **depends upon**. Each item is
stated as precisely as possible, given a stable identifier (`T1`, `T2`, …), a status tag, a dependency list, and a note
on whether a proof is known, trivial, classical, missing, or whether the statement is _false as currently written_ and
needs repair.
...
| `[X]` | **False or incorrect as stated** in the source documents. Correction supplied. |
...
presentation. The
"generations" \(\{2\},\{3\},\{5,7\},\{11,\dots\}\) of `generator.md` §2.3 are a labelling of the bands \([P_{k-1},
P_k)\) or \([N, N^2]\) depending on which recursion is used; **the two are not the same recursion** and the source
document conflates them slightly. See T14 for the primorial version.
...
## Part III — The Multiplier-List Theory (`observation.md`), Corrected

This part is the most consequential: the observation as written is **wrong from \(N = 11\) onward**, but the correct
version is _better_ than the stated one, because it removes the "weird boundary" problem entirely.

### T17. The observation as stated `[X]`

`observation.md` claims the multiplier list for prime \(N\) is \(A_N = \{1\} \cup \{m \text{ prime} : N \le m < P_{<
N}\}\).

**This is false.** Counterexample \(N = 11\), \(P_{<11} = 210\): the multiplier \(121 = 11^2\) is required (since \(11
\cdot 121 = 1331 = 11^3\) has smallest prime factor 11 and lies in the first period), but \(121\) is not prime and is
excluded by the stated rule. Likewise \(143 = 11\cdot13\), \(169 = 13^2\), \(187 = 11\cdot17\), \(209 = 11\cdot19\) are
missing. The stated list has \(43\) elements; the correct count is \(\varphi (210) = 48\).

### T18. The correct multiplier list `[T]`

\[ A_N \;=\; \{\, t \in [1, P_{<N}] \;:\; \gcd (t, P_{<N}) = 1 \,\}, \qquad |A_N| = \varphi (P_{<N}) = \!\!\prod_{p <
N}\!\! (p-1). \] i.e. the multipliers are the **totatives of the primorial**, not the primes. _Proof:_ T15.

### T19. Why the observation looked right `[T]`

A totative of \(P_{<N}\) that is \(> 1\) and \(< N^2\) must be prime. _Proof:_ a composite totative has \(\ge 2\) prime
factors, each \(\ge N\), hence is \(\ge N^2\). **Therefore the prime-only rule of T17 is exactly correct iff \(P\_{<N}
\le N^2\).**
\(P_{<N} \le N^2\) holds for \(N = 2,3,5,7\) (\(30 \le 49\)) and fails for every \(N \ge 11\) (\(210 > 121\)). All four
worked examples in `observation.md` are inside the coincidence window; the first untested case is the first failure.
...
### T21. Dissolution of the "weird boundary" `[X]` → `[T]`

`observation.md` §"The Weird Boundary for Prime Inference" asserts that the multiplier list of \(N\) depends on primes
discovered _after_ \(N\) (all primes up to \(P_{<N}\)), forcing mutable, partially-computed, open-ended lists.

**This dependence is an artifact of the incorrect formulation T17.** Under T18 the multiplier set is the reduced residue
system mod \(P_{<N}\), which is determined entirely by the primes **strictly less than \(N\)** — all of which are known
at the moment \(N\) is discovered. There is no forward dependence, no partially-computed state, and no need for mutable
retroactive updates.

_This is the single most important correction in this document._ It converts an apparently non-causal specification into
a causal, streamable one, which is a precondition for the O (1) pointer rule to be even conceivable.
...
sequence". As a description of _residue classes_ that is complete (T15); as a description of the _composite stream_ it
is a truncation and must be extended.
...
_Conclusion:_ **"O (1) `NextOwnedComposite`" as stated in `generator.md` §3 is true but vacuous unless the space cost is
bounded.** The honest form of the assumption is:
...
### T32. Truncated / hybrid wheels break orthogonality `[T]`
...
(c) total heap traffic rises from \(X - \pi (X)\) to \(\sum_{p\le\sqrt X}\Phi (X/p, p_j)\). _This is a required
correction to §4.2 of `generator.md` for any practical implementation._
...
### T38. Lazy insertion bound `[T]`

A prime \(p\) contributes nothing below \(p^2\); therefore, if insertion is deferred until \(n\) reaches \(p^2\), the
heap holds only \(\pi (\sqrt{n})\) entries. _Consequence:_ memory is \(O (\sqrt{n}/\log n)\) for the heap, not \(O (\pi
(n))\) as claimed in
`generator.md` §4.5. **The paper understates its own memory result.**
(The full prime _list_ is still \(O (\pi (n))\), but only \(\pi (\sqrt n)\) of it is needed for generation.)
...
### T41. Branchlessness claim `[?]`

`generator.md` §5.2 claims the inner loop is effectively branchless. _Status:_ unproven and, as written, doubtful — the
heap sift is data-dependent and branch-heavy. The claim is defensible only in the T39 bucketed form, and even then it is
an engineering claim, not a theorem. Should be softened or benchmarked.
...
With a heap: \(O (X \log \pi (\sqrt X)) = O (X \log\log X)\) — note this is _better_ than the \(O (X\log X)\) stated in
`generator.md` §4.5, which incorrectly uses \(\pi (X)\) rather than \(\pi (\sqrt X)\) for the heap size (see T38). With
T39 buckets: \(O (X)\). _Comparison:_ segmented Eratosthenes is \(O (X\log\log X)\); Pritchard's wheel sieve is \(O
(X/\log\log X)\); the linear (Gries–Misra) sieve is \(O (X)\). So the architecture is competitive but **not
asymptotically new**.
...
available here are (a) the explicit key-uniqueness property T37, (b) the memory bound T38 that the source paper
undersells, and (c) the causality correction T21, which is a real fix to a real error.
...
2. **T33.** Design and prove the three-regime hybrid pointer, with exact orthogonality preserved across regime
   boundaries.
3. **T32.** If truncated wheels are used, quantify the over-claim rate and prove the modified consuming loop
   (drain-all-equal-keys) still correct and still \(O (X)\).
4. **T41.** Substantiate or retract the branchlessness / SIMD / GPU claims.
5. **T43.** Tighten and re-derive the complexity statement in `generator.md` §4.5, which currently uses the wrong heap
   size.
...
## Part X — Dependency Summary
    ...
    T18 ─┬─> T19 (explains why observation.md failed at N=11)
         ├─> T20, T22
         └─> T27 ─> T28 ─> T29 ─> T30
...
one-liners. The description in `observation.md` is wrong in a specific, fixable way (T17/T18/T19), and fixing it
eliminates the paper's own
"weird boundary" problem (T21). The single genuine unknown is T31: the O (1) pointer rule is trivially achievable in
time but only at primorial space cost, so `generator.md`'s central assumption must be restated as a conjecture with an
explicit space bound before any of its performance claims can be taken seriously.
