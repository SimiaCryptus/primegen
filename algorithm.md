...
5. **Algorithm C** (§4C, full specification in `min_factor.md`) re-indexes B by the **min-factor normal form**
   ...
   The multiplier set C actually streams is the **wheeled** relaxation \(\tilde B_p\), not \(B_p\): a \(W\)-coprime
   wheel that still contains composites, so C over-claims exactly as B does (§4C.1, `min_factor.md` Remark M-R).
...
> Three warnings. Theorem 2.10 does not change the partition — summing the layers re-assembles \(p\cdot A_p = \Theta_p\) —
...
> And Lemma 2.12 is about the **exact** cloud \(B_p\) only: the wheeled cloud \(\tilde B_p\) of §4C.1 contains
> composites, so a layer's multipliers below \(p^2\) are not primes and the emitted value need not reach \(p^{e+2}\)
> (\(11\cdot 49 = 539 = 7^2\cdot 11 < 11^3\) at \(W=30\)). See `min_factor.md` Remark M-R.
...
layer, and are exactly what Lemma 4C.5 counts (`min_factor.md` Remark M-R). Heads:
